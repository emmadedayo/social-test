...
const isAuth = async (req, res, next) => {
    ...
    jsonwebtoken_1.default.verify(token, configs_1.environmentConfig.JWT_TOKEN_SECRET, async (err, decodedUser) => {
        if (err) {
        ...
        try {
            const decodedUserInDB = await User_Model_1.default.findOne({ _id: decodedUser?.user._id });
            if (!decodedUserInDB) {
                return next((0, http_errors_1.default)(403, `sAuth Failed (Unauthorized)`));
            }
